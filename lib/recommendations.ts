...
export function scoreCollectionRecommendations(user: User, collection: CollectionWithFrag[], catalog: Fragrance[]) {
  ...
  user.favoriteNotes.forEach((n) => noteWeights.set(n, (noteWeights.get(n) ?? 0) + 3));
  user.styleTags.forEach((v) => vibeWeights.set(v, (vibeWeights.get(v) ?? 0) + 2));

  collection.forEach((item) => {
    const weight = (item.rating ?? 6) + item.wearCount * 0.2;
    item.fragrance.notes.forEach((n) => noteWeights.set(n, (noteWeights.get(n) ?? 0) + weight));
    item.fragrance.vibeTags.forEach((v) => vibeWeights.set(v, (vibeWeights.get(v) ?? 0) + weight));
  });

  const seasonCoverage = new Set(collection.flatMap((c) => c.fragrance.seasonTags));
  ...
  return catalog
    ...
    .map((f) => {
      ...
      if (!seasonCoverage.has("summer") && f.seasonTags.includes("summer")) score += 15;
      if (!seasonCoverage.has("winter") && f.seasonTags.includes("winter")) score += 12;

      const sweetHeavy = (vibeWeights.get("sweet") ?? 0) > (vibeWeights.get("fresh") ?? 0) * 1.2;
    ...
    .map((f) => {
      ...
      if (weather) {
        if (weather.tempF >= 80 || weather.humidity >= 75) {
          if (f.vibeTags.some((v) => ["fresh", "clean"].includes(v)) || f.notes.some((n) => ["citrus", "aquatic"].includes(n))) {
            score += 20;
            reason = `It's ${Math.round(weather.tempF)}°F and humid — this fresh citrus-musky profile stays clean.`;
          }
        } else if (weather.tempF <= 50) {
          if (f.notes.some((n) => ["vanilla", "amber", "spice", "woody"].includes(n))) {
            score += 20;
            reason = "Cold day + date night — warm vanilla/amber with strong longevity.";
          }
        } else if (weather.rainy && f.notes.some((n) => ["tea", "musk", "woods"].includes(n))) {
          score += 16;
